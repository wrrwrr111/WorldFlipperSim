...
const SilverPin = preload('silver_pin.tscn')
const GoldPin = preload('gold_pin.tscn')
const RainbowPin = preload('rainbow_pin.tscn')
const Ball = preload('ball/ball.tscn')
	...
	for i in 4:
		for j in 15:
			if randi()%100 < silver_chance:
				var silver_pin = SilverPin.instance()
				var x = 48+150*i + (j%2)*75
				var y = 500+100*j
				silver_pin.set_position(Vector2(x,y))
				add_child(silver_pin)
				pin_arr.append(silver_pin)
	for i in randi()%gold_more:
		var gold_pin = GoldPin.instance()
		gold_pin.set_position(Vector2(randi()%600,800+randi()%1200))
		add_child(gold_pin)
		pin_arr.append(gold_pin)
	for i in randi()%rainbow_more:
		var rainbow_pin = RainbowPin.instance()
		rainbow_pin.set_position(Vector2(randi()%600,1200+randi()%800))
		add_child(rainbow_pin)
		pin_arr.append(rainbow_pin)
